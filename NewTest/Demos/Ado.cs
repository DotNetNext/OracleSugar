    ...
    public class Ado:IDemos
    {
        ...
        public void Init()
        {
            ...
            using (var db = SugarDao.GetInstance())
            {
                ...
                var r2 = db.GetSingle<Student>("select   * from student where rownum<=1");
                var r3 = db.GetScalar("select  count(1) from student");
                var r4 = db.GetReader("select  count(1) from student ");
                r4.Dispose();
                var r5 = db.GetString("select   name from student  where rownum<=1");
                var r6 = db.ExecuteCommand("select 1 from dual");
                ...
                var p3 = db.GetDataTable("select * from student where id=:id", new OracleParameter(":id", 1));
